...
turtles-own [
  virus? ;;check the agent has virus (true or false)
  dead? ;;check our agent is alive (true or false)
  immune? ;;check our agent is immune (true or false)
  vaccinated? ;;check our agent is vaccinated
  duration ;;a counter for the duration of the virus in the agent
  quarantined? ;;check the agent is in quarantine (true or false)
]

;;this procedure called "setup" sets up the simulation for start
to setup
  clear-all ;;clear the environment before of starting
  set-patch-size (300 / max-pxcor) ;;adjust the display dimensions
...
to go
  ;;if all turtles have virus, we will stop simulation
  if all? turtles [virus?] = true [stop]
  ;;if no turtles have virus, we will stop simulation
  if all? turtles [virus? = false] = true [stop]
  ;;if no more turtles to be vaccinated or dead, we will stop simulation
  if count turtles with [(vaccinated? = false and dead? = false)] < vaccines-per-day = true [stop]

  ;;We want turtles to spread and to do their normal life:
  ask turtles [move]
  ...
  ;;we want that our agents recover/die
  ask turtles [recover]
...
to spread
  ...
  [
    ...
    [
      ...
      [
        if (random-float 1 < (probabilityOfTransmission * temp))
        [ ;;compare the probabilityOfTransmission (that can be changed with the slider)
...
to recover
  ...
  [
    ...
    if random-float 1.0 < recoverProbability and duration > 10
    [ ;;this is the chance to recover
    ...
    if random-float 1.0 < deathProbability and duration > 5
    [ ;;this is the chance to die (i put deathProbability, you can change in the slider)
      set dead? true
    ]
  ]
end
